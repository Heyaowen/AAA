...
def run():
    ...
    if not is_online(auto_login=True):
        print('程序已退出...')
        m_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        print('当前时间：{}'.format(m_date))
        return
...
def is_online(auto_login=False):
    ...
    def _online():
        """
        通过获取好友信everyday_wechat息，判断用户是否还在线。
        :return: bool,当返回为 True 时，在线；False 已断开连接。
        """
        try:
...
def init_alarm(alarm_dict):
    ...
    scheduler.add_job(refresh_wechat,'interval',minutes=28)
...
@itchat.msg_register([CARD,NOTE,SHARING,ATTACHMENT])
def text_reply(msg):
    """ 监听用户消息，用于自动回复 """
    handle_friend(msg)
    # 下面这段代码，可以很直观打印出返回的消息的数据结构。
